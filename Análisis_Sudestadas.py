...
tabla_eventos = pd.DataFrame(eventos,
                             columns=["Inicio", "Fin", "Duración", "Altura máxima alcanzada (m)"])
...
tabla_eventos = tabla_eventos[
    ["Inicio", "Fin", "Duración (horas)", "Altura máxima alcanzada (m)"]
]
...
# Convertir 0–360 → -180–180 si aplica
...
for i in range(len(tabla_eventos)):
    ...
    alt_max = tabla_eventos.loc[i, "Altura máxima alcanzada (m)"]
...
for i in range(3):
    ...
    ax.quiver(
        ...
        transform=ccrs.PlateCarree(),
        scale=200, scale_units='height', color='black'
    )
...
# Creación de arrays de valores True/False
lat_mask = (lats >= lat_min) & (lats <= lat_max)
lon_mask = (lons >= lon_min) & (lons <= lon_max)

# Subconjuntos de coordenadas: utilizamos los arrays booleanos para indexar
